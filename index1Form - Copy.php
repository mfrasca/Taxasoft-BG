...
{
	global $cfg, $tableNr, $tableName, $message, $start, $dbase, $maxFieldLen, $sort, $find, $search, $parent_id, $action, $debug;
	...
	foreach( $cfg->table[$tableNr]->field as $field)
	{
		...
		$fieldTableNr= false;
		$fieldValue= str_replace('"', "&quot;", stripslashes( $row[ "$fieldName" ]) );	//** replace double quotes otherwise you lose part of the data in that field
		...
		else
		{
			...
			if( $fieldType == 'select')
			{
				...
				else if( isset( $field['tableNr']))
				{
					...
					if( $field['noEdit'])
						$fieldValueRef= getReference( (int)$field['tableNr'], 'id', $row["$fieldName"]);
					...
					else
					{
						...
						if( $field['dependent'])
						{
							if( $pos= strpos( $field['dependent'], "="))
							{
								...
								$depFieldValue= $row[ substr( $field['dependent'], $pos+1) ];
							}
							else
							{
								$depFieldName = $field['dependent'];
								$depFieldValue= $row["{$field['dependent']}"];
							}
						...
						while( $sRow = mysql_fetch_array($sResult) )
						{
							foreach( $refFieldArray as $refField)
							{
								...
								if( $tableNrSelectFields[$refField] )					//* if this is a select field to an other table
				...
				if( $field['noEdit'])
				{
					...
					showSelectionField( $array, $fieldName, $row["$fieldName"], $field['byValue'], $field['onchange'] );
			...
			else	//** normal field **/
			{
				...
				if( $fieldName  == 'autoIndex')
					echo "
						<tr><td>$showFieldNameAs</td><td><b>{$row["$fieldName"]}</b></td></tr>";
				...
				else if( $fieldName  != 'id')
				{
					...
					if( $fieldName == 'url' && $row[ "$fieldName" ] != '' && !$field['noEdit'])
					{
...
function showButtons( $tableNr, $row)
{
	...
	foreach( $cfg->table[$tableNr]->function as $function)
	{
		...
		$url= $function['url'];
		$array= explode( ',', $function['replace']);
